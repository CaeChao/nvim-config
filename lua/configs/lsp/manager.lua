...
local function resolve_config(server_name, ...)
  local defaults = {
    ...
    capabilities = require("configs.lsp").common_capabilities(),
  }

  local has_custom_provider, custom_config = pcall(require, "configs/lsp/providers/" .. server_name)
...
local function client_is_configured(server_name, ft)
  ...
  for _, result in ipairs(active_autocmds) do
    if result.desc ~= nil and result.desc:match("server " .. server_name .. " ") then
      Log:info(string.format("[%q] is already configured", server_name))
      return true
    end
  end
  return false
...
local function launch_server(server_name, config)
  pcall(function()
    ...
    if type(command) == "table" and type(command[1]) == "string" and vim.fn.executable(command[1]) ~= 1 then
      Log:warn(string.format("[%q] is either not installed, missing from PATH, or not executable.", server_name))
      return
    end
...
function M.setup(server_name, user_config)
  ...
  if not pkg_name then
    Log:warn(server_name .. " is not managed by the Mason")
